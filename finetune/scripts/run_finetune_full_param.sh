...
# LoRA configuration
LORA_R=16  # Smaller rank may help prevent overfitting
LORA_ALPHA=32  # Keep same ratio (alpha/r = 2)
...
CMD="torchrun --nproc_per_node=$NUM_GPUS --master_port=$MASTER_PORT scripts/train_lora_full_param.py \
    --seq-length $SEQ_LENGTH \
    --data-path $DATA_PATH \
    --data-cache-path $DATA_CACHE_PATH \
    --split $DATA_SPLIT \

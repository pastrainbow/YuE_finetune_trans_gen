...
# Prompt
parser.add_argument("--track_info_json", type=str, required=True, help="The file path to a json file that contains the splitting endpoints and genres for each prompt track.")
...
parser.add_argument("--audio_prompts_dir_path", type=str, default="", help="The directory path for the split audio files to use as reference prompts.")
...
if args.use_audio_prompt and not args.audio_prompts_dir_path:
...
def stage1_inference(model, 
    ...
    for i, p in enumerate(tqdm(prompt_texts[:run_n_segments], desc=f"Track {track_name} stage1 inference...")):
        ...
        if i==1:
            if args.use_dual_tracks_prompt or args.use_audio_prompt:
                if args.use_dual_tracks_prompt:
                    ...
                    audio_prompt = prompts_concat(start_audio_prompt_path, end_audio_prompt_path, args.gen_duration, device)

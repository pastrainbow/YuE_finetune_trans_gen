...
def silence_gen(frame_count, device):    
    ...
    return torch.zeros((frame_count,), device=device)
...
def prompts_concat(start_audio_path, end_audio_path, noise_duration, device, sample_rate=16000):
    ...
    middle_silence = silence_gen(
        int(noise_duration * sample_rate),
        device,
    )

    concat_data = torch.cat((start_audio_data, middle_silence, end_audio_data), dim=0)
    ...
    if i==1:
        if args.use_dual_tracks_prompt or args.use_audio_prompt:
            if args.use_dual_tracks_prompt:
                ...
                token_count = raw_codes[0].shape[1]
                print(f"[DEBUG] token count: {token_count}")
                
                # np.set_printoptions(threshold=np.inf)
                # print(f"Raw codes: {raw_codes}, with shape {raw_codes.shape}")
                ...
                id_count = len(code_ids)

                segment_id_count = id_count // 3

                code_ids[segment_id_count : segment_id_count * 2] = [mmtokenizer.mask] * segment_id_count

                print(f"[DEBUG] code ids after masking: {code_ids}")
